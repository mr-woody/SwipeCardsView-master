...
public class SwipeCardsView extends FrameLayout {
    ...
    public SwipeCardsView(Context context, AttributeSet attrs, int defStyle) {
        ...
        btnListener = new OnClickListener() {
            ...
            public void onClick(View view) {
                if (null != mCardsSlideListener && ViewHelper.getScaleX(view) == 1f) {
    ...
    private static final Interpolator sInterpolator = new Interpolator() {
        ...
        public float getInterpolation(float t) {
            return (float)(Math.cos((t + 1) * Math.PI) / 3.0f) + 0.5f;
        }
    };
...
//            addView(changedView,0);
    ...
    private void ajustLinkageViewItem(View changedView, float rate, int index) {
        ...
        View ajustView = viewList.get((changeIndex + index)<viewList.size()?changeIndex + index:viewList.size()-1);
        ajustView.offsetTopAndBottom(offset - ajustView.getTop() + initTop);

        ViewHelper.setScaleX(ajustView,scale);
        ViewHelper.setScaleY(ajustView,scale);
        ViewHelper.setAlpha(ajustView,alpha);
    }
    ...
    private void animToSide(View changedView, float xvel, float yvel) {
        float finalX = initLeft;
        float finalY = initTop;
        ...
        if (xvel > X_VEL_THRESHOLD && Math.abs(yvel) < xvel * xyRate) {
            // x正方向的速度足够大，向右滑动消失
            finalX = mWidth;
            finalY = yvel * (mCardWidth + changedView.getLeft()) / xvel + changedView.getTop();
        ...
        if (finalX == initLeft) {
            ((CardItemView)changedView).animTo(initLeft, initTop);
        } else {
            // 2. 向两边消失的动画
            startScrollTopView((int) finalX, (int)finalY, SCROLL_DURATION, flyType);
        }
    }
